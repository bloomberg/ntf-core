...
#ifndef INCLUDED_NTCO_POLL
#define INCLUDED_NTCO_POLL
...
/// @internal @brief
/// Provide a factory to produce reactors implemented using the "poll" API.
///
/// @details
/// This class implements the 'ntci::ReactorFactory' interface to produce
/// reactors implemented using the "poll" API.
///
/// @par Thread Safety
/// This class is thread safe.
///
/// @ingroup module_ntco
class PollFactory : public ntci::ReactorFactory
{
    bslma::Allocator* d_allocator_p;

  private:
    PollFactory(const PollFactory&) BSLS_KEYWORD_DELETED;
    PollFactory& operator=(const PollFactory&) BSLS_KEYWORD_DELETED;

  public:
    /// Create a new reactor factory that produces reactors implemented
    /// using the "poll" API. Optionally specify a 'basicAllocator'
    /// used to supply memory. If 'basicAllocator' is 0, the currently
    /// installed default allocator is used.
    explicit PollFactory(bslma::Allocator* basicAllocator = 0);

    /// Destroy this object.
    ~PollFactory() BSLS_KEYWORD_OVERRIDE;
