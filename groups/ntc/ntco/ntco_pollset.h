...
#ifndef INCLUDED_NTCO_POLLSET
#define INCLUDED_NTCO_POLLSET
...
/// @internal @brief
/// Provide a factory to produce reactors implemented using the "pollset"
/// API.
///
/// @details
/// This class implements the 'ntci::ReactorFactory' interface to produce
/// reactors implemented using the "pollset" API.
///
/// @par Thread Safety
/// This class is thread safe.
///
/// @ingroup module_ntco
class PollsetFactory : public ntci::ReactorFactory
{
    bslma::Allocator* d_allocator_p;

  private:
    PollsetFactory(const PollsetFactory&) BSLS_KEYWORD_DELETED;
    PollsetFactory& operator=(const PollsetFactory&) BSLS_KEYWORD_DELETED;

  public:
    /// Create a new reactor factory that produces reactors implemented
    /// using the "pollset" API. Optionally specify a 'basicAllocator'
    /// used to supply memory. If 'basicAllocator' is 0, the currently
    /// installed default allocator is used.
    explicit PollsetFactory(bslma::Allocator* basicAllocator = 0);

    // Destroy this object.
    ~PollsetFactory() BSLS_KEYWORD_OVERRIDE;
