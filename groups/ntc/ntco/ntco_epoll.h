...
#ifndef INCLUDED_NTCO_EPOLL
#define INCLUDED_NTCO_EPOLL
...
/// @internal @brief
/// Provide a factory to produce reactors implemented using the "epoll" API.
///
/// @details
/// This class implements the 'ntci::ReactorFactory' interface to produce
/// reactors implemented using the "epoll" API.
///
/// @par Thread Safety
/// This class is thread safe.
///
/// @ingroup module_ntco
class EpollFactory : public ntci::ReactorFactory
{
    bslma::Allocator* d_allocator_p;

  private:
    EpollFactory(const EpollFactory&) BSLS_KEYWORD_DELETED;
    EpollFactory& operator=(const EpollFactory&) BSLS_KEYWORD_DELETED;

  public:
    /// Create a new reactor factory that produces reactors implemented
    /// using the 'epoll' API. Optionally specify a 'basicAllocator'
    /// used to supply memory. If 'basicAllocator' is 0, the currently
    /// installed default allocator is used.
    explicit EpollFactory(bslma::Allocator* basicAllocator = 0);

    /// Destroy this object.
    ~EpollFactory() BSLS_KEYWORD_OVERRIDE;
