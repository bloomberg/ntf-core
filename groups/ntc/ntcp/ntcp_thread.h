...
#ifndef INCLUDED_NTCP_THREAD
#define INCLUDED_NTCP_THREAD
...
/// @internal @brief
/// Provide asynchronous, multiplexed sockets and timers using a proactor
/// driven by a single thread.
///
/// @details
/// Provide a mechanism that runs a thread to automatically block
/// on a proactor to implement the asynchronous behavior required by sockets
/// and timers.
///
/// @par Thread Safety
...
class Thread : public ntci::Thread, public ntccfg::Shared<Thread>
{
  ...
  private:
    /// Wait for I/O events.
    static void* run(ntcp::Thread* thread);
  ...
  public:
    ...
    /// a proactor produced by the specified 'proactorFactory'. Optionally
    ...
    /// proactor driven by this thread. Optionally specify a 'basicAllocator'
