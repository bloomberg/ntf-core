...
#ifndef INCLUDED_NTCP_DATAGRAMSOCKET
#define INCLUDED_NTCP_DATAGRAMSOCKET
                       ...
                       public ntccfg::Shared<DatagramSocket>
{
  ...
  private:
    ...
    void privateInitiateReceive(const bsl::shared_ptr<DatagramSocket>& self);

    /// Process the completion of the reception of a message from the
    /// specified 'endpoint' having the specified 'data'. The behavior is
    /// undefined unless 'd_mutex' is locked.
    void privateCompleteReceive(const bsl::shared_ptr<DatagramSocket>& self,
                                const ntsa::Endpoint&               endpoint,
                                const bsl::shared_ptr<bdlbb::Blob>& data);

    /// Process the failure of the reception of a message. The behavior is
    /// undefined unless 'd_mutex' is locked.
    void privateFailReceive(const bsl::shared_ptr<DatagramSocket>& self,
                            const ntsa::Error&                     error);

    /// Initiate a new transmission of the message at the head of the write
    /// queue, if allowed and necessary.
    void privateInitiateSend(const bsl::shared_ptr<DatagramSocket>& self);

    /// Process the completion of the transmission of the message at the
    /// head of the write queue. The behavior is undefined unless 'd_mutex'
    /// is locked.
    void privateCompleteSend(const bsl::shared_ptr<DatagramSocket>& self);
    ...
    void privateFailSend(const bsl::shared_ptr<DatagramSocket>& self,
                         const ntsa::Error&                     error);

    /// Indicate a failure has occurred and detach the socket from its
    /// proactor. The behavior is undefined unless 'd_mutex' is locked.
    ...
    /// Allocate a new blob assigned to 'd_receiveFromBlob_sp', if necessary
  ...
  public:
    ...
    /// Cancel the send-to operation identified by the specified 'token'.
    /// Return the error.
    ntsa::Error cancel(const ntca::SendToken& token) BSLS_KEYWORD_OVERRIDE;

    /// Cancel the receive-from operation identified by the specified
    /// 'token'. Return the error.
