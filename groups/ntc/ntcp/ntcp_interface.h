...
#ifndef INCLUDED_NTCP_INTERFACE
#define INCLUDED_NTCP_INTERFACE
...
/// @internal @brief
/// Provide asynchronous, multiplexed sockets and timers using proactors driven
/// by a pool of threads.
///
/// @details
/// Provide a mechanism that runs a pool of threads to
/// automatically block on a single proactor (if dynamically load balancing) or
/// set of proactors (if statically load balancing) to implement the
                  ...
                  public ntccfg::Shared<Interface>
{
    ...
    /// identifiers as unsigned 64-bit integers to proactors driven by those
    /// threads.
    typedef bsl::unordered_map<bsl::uint64_t, bsl::shared_ptr<ntci::Proactor> >
        ThreadMap;
  ...
  private:
    ...
    /// Add a new proactor. Return the new proactor.
    bsl::shared_ptr<ntci::Proactor> addProactor();

    /// Add a new thread. Return the error.
    ntsa::Error addThread();

    /// Acquire usage of the proactor used by the thread identified by the
    /// specified 'options.threadHandle()' and increment the estimated load
    /// on that proactor by the specified 'options.weight()'. Return the
    /// proactor acquired, or null if no such thread identified by
    /// 'threadHandle' can be found.
    bsl::shared_ptr<ntci::Proactor> acquireProactorUsedByThreadHandle(
        const ntca::LoadBalancingOptions& options);

    /// Acquire usage of the proactor used by the thread stored at the
    /// specified 'options.threadIndex()' modulo the total number of threads
    /// and increment the estimated load on that proactor by the specified
    /// 'options.weight()'. Return the proactor acquired, or null if no such
    /// thread is stored at the thread index.
    bsl::shared_ptr<ntci::Proactor> acquireProactorUsedByThreadIndex(
        const ntca::LoadBalancingOptions& options);

    /// Acquire usage of the proactor with the least amount of load and
    /// increment the estimated load on that proactor by the specified
    /// 'options.weight()'. Automatically expand the thread pool if all
    /// proactors have a load greater than or equal to the configured maximum
    /// desired load per proactor, and the current number of threads is less
    /// than the configured maximum number of threads. Return the proactor
    /// acquired.
  ...
  public:
    ...
    /// proactors using the specified 'proactorFactory'. Optionally specify a
    ...
    /// Acquire usage of the most suitable proactor selected according to the
