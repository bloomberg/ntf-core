...
#ifndef INCLUDED_NTCI_RECEIVECALLBACKFACTORY
#define INCLUDED_NTCI_RECEIVECALLBACKFACTORY
...
class ReceiveCallbackFactory
{
  public:
    /// Destroy this object.
    virtual ~ReceiveCallbackFactory();

    /// Create a new receive callback to invoke the specified 'function' with
    /// no cancellable authorization mechanism on this object's strand.
    /// Optionally specify a  'basicAllocator' used to supply memory. If
    /// 'basicAllocator' is 0, the currently installed default allocator is
    /// used.
    ntci::ReceiveCallback createReceiveCallback(
        const ntci::ReceiveFunction& function,
...
NTCCFG_INLINE
ntci::ReceiveCallback ReceiveCallbackFactory::createReceiveCallback(
    const ntci::ReceiveFunction& function,
    bslma::Allocator*            basicAllocator)
{
    return ntci::ReceiveCallback(function, this->strand(), basicAllocator);
}
...
ntci::ReceiveCallback ReceiveCallbackFactory::createReceiveCallback(
    ...
    bslma::Allocator*                           basicAllocator)
{
    return ntci::ReceiveCallback(function,
                                 authorization,
                                 this->strand(),
                                 basicAllocator);
}
...
ntci::ReceiveCallback ReceiveCallbackFactory::createReceiveCallback(
    ...
    bslma::Allocator*                    basicAllocator)
{
    return ntci::ReceiveCallback(function, strand, basicAllocator);
}
