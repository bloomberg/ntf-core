...
#ifndef INCLUDED_NTCI_REACTORPOOL
#define INCLUDED_NTCI_REACTORPOOL
...
/// Provide a pool of reactors within which sockets are load balanced.
///
/// @par Thread Safety
...
class ReactorPool
{
  public:
    ...
    /// Acquire usage of the most suitable reactor selected according to
    /// the specified load balancing 'options'.
    virtual bsl::shared_ptr<ntci::Reactor> acquireReactor(
    ...
    virtual void releaseReactor(const bsl::shared_ptr<ntci::Reactor>& reactor,
                                const ntca::LoadBalancingOptions& options) = 0;
    ...
    virtual bsl::size_t numReactors() const = 0;
