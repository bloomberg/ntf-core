...
#ifndef INCLUDED_NTCI_REACTORMETRICS
#define INCLUDED_NTCI_REACTORMETRICS
...
/// Provide an interface to record the runtime behavior of reactors.
///
/// @par Thread Safety
/// This class is thread safe.
///
/// @ingroup module_ntci_reactor
class ReactorMetrics : public ntci::Monitorable
{
  public:
    /// Destroy this object.
    virtual ~ReactorMetrics();
...
#define NTCI_REACTORMETRICS_GET()                                             \
    ntci::ReactorMetrics* metrics = result->d_metrics_sp.get()

#define NTCI_REACTORMETRICS_UPDATE_INTERRUPTS(numSignals)                     \
    if (metrics) {                                                            \
        metrics->logInterrupt(numSignals);                                    \
    }

#define NTCI_REACTORMETRICS_UPDATE_POLL(numReadable, numWritable, numErrors)  \
    if (metrics) {                                                            \
        metrics->logPoll(numReadable, numWritable, numErrors);                \
    }

#define NTCI_REACTORMETRICS_UPDATE_DEFERRED_SOCKET()                          \
    if (metrics) {                                                            \
        metrics->logDeferredSocket();                                         \
    }

#define NTCI_REACTORMETRICS_UPDATE_SPURIOUS_WAKEUP()                          \
    if (metrics) {                                                            \
        metrics->logSpuriousWakeup();                                         \
    }

#define NTCI_REACTORMETRICS_UPDATE_ERROR_CALLBACK_TIME_BEGIN()                \
    bsl::int64_t errorProcessingStartTime;                                    \
    if (metrics) {                                                            \
        errorProcessingStartTime = bsls::TimeUtil::getTimer();                \
    }

#define NTCI_REACTORMETRICS_UPDATE_ERROR_CALLBACK_TIME_END()                  \
    if (metrics) {                                                            \
...
#define NTCI_REACTORMETRICS_UPDATE_WRITE_CALLBACK_TIME_BEGIN()                \
    bsl::int64_t writeProcessingStartTime;                                    \
    if (metrics) {                                                            \
        writeProcessingStartTime = bsls::TimeUtil::getTimer();                \
    }

#define NTCI_REACTORMETRICS_UPDATE_WRITE_CALLBACK_TIME_END()                  \
    if (metrics) {                                                            \
...
#define NTCI_REACTORMETRICS_UPDATE_READ_CALLBACK_TIME_BEGIN()                 \
    bsl::int64_t readProcessingStartTime;                                     \
    if (metrics) {                                                            \
        readProcessingStartTime = bsls::TimeUtil::getTimer();                 \
    }

#define NTCI_REACTORMETRICS_UPDATE_READ_CALLBACK_TIME_END()                   \
    if (metrics) {                                                            \
