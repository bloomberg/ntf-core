...
#ifndef INCLUDED_NTCA_STREAMSOCKETOPTIONS
#define INCLUDED_NTCA_STREAMSOCKETOPTIONS
...
/// Describe the configuration of a stream socket.
///
/// @par Attributes
...
class StreamSocketOptions
{
  ...
  public:
    ...
    StreamSocketOptions& operator=(const StreamSocketOptions& other);

    /// Set the transport to the specified 'value'.
    void setTransport(ntsa::Transport::Value value);

    /// Set the remote endpoint of the stream socket to the specified
    /// 'value'.
    ...
    /// than a graceful shutdown, otherwise return false.
    bool abortiveClose() const;
...
///
/// @related ntca::StreamSocketOptions
bool operator==(const StreamSocketOptions& lhs,
                const StreamSocketOptions& rhs);
