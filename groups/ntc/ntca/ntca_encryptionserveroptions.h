...
/// the server role. Such 'ntca::Encryption' interfaces are subsequently used
/// to passively wait for the peer to initiate a cryptographically secure
...
///
/// @li @b identityData:
/// The PEM-encoded certificate containing the server's identity, public key,
/// and issuer. If defined, the identity data takes precedence over the
/// identity file.
///
/// @li @b identityFile:
/// The path to the PEM-encoded certificate on disk containing the server's
/// identity, public key, issuer, etc.
///
/// @li @b privateKey:
/// The private key of the server. If defined, the private key object takes
/// precendence over the private key data.
///
/// @li @b privateKeyData:
/// The PEM-encoded private key of the server. If defined the private key data
/// takes precedence over the private key file.
///
/// @li @b privateKeyFile:
/// The path to the PEM-encoded private key of the server.
///
/// @li @b authorityList:
...
class EncryptionServerOptions
{
  ...
  public:
    /// Create new encryption server options.
    ...
    /// Return the PEM-encoded certificate containing the server's
    /// identity, public key, and issuer.
    const bdlb::NullableValue<bsl::vector<char> >& identityData() const;

    /// Return the file path to the certificate containing the server's
    /// identity, public key, and issuer.
    ...
    /// Return the file path to the server's private key.
