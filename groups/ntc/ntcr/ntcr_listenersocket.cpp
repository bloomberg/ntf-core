...
BSLS_IDENT_RCSID(ntcr_listenersocket_cpp, "$Id$ $CSID$")
...
#define NTCR_LISTENERSOCKET_LOG_BACKLOG_THROTTLE_APPLIED(timeToSubmit)        \
    NTCI_LOG_TRACE("Listener socket backlog throttle applied for %d "         \
                   "milliseconds",                                            \
                   (int)((timeToSubmit).totalMilliseconds()))

#define NTCR_LISTENERSOCKET_LOG_BACKLOG_THROTTLE_RELAXED()                    \
    NTCI_LOG_TRACE("Listener socket backlog throttle relaxed")

#define NTCR_LISTENERSOCKET_LOG_BACKLOG_UNDERFLOW()                           \
    NTCI_LOG_TRACE("Listener socket has emptied the backlog")

#define NTCR_LISTENERSOCKET_LOG_ACCEPT_RESULT(remoteEndpoint)                 \
...
#define NTCR_LISTENERSOCKET_LOG_ACCEPT_QUEUE_FILLED(size)                     \
    NTCI_LOG_TRACE("Listener socket "                                         \
                   "has filled the accept queue up to %zu connections",       \
                   size)

#define NTCR_LISTENERSOCKET_LOG_ACCEPT_QUEUE_DRAINED(size)                    \
    NTCI_LOG_TRACE("Listener socket "                                         \
                   "has drained the accept queue down to %zu connections",    \
                   size)

#define NTCR_LISTENERSOCKET_LOG_ACCEPT_QUEUE_LOW_WATERMARK(lowWatermark,      \
                                                           size)              \
    NTCI_LOG_TRACE("Listener socket "                                         \
...
#define NTCR_LISTENERSOCKET_LOG_ACCEPT_QUEUE_HIGH_WATERMARK(highWatermark,    \
                                                            size)             \
    NTCI_LOG_TRACE("Listener socket "                                         \
...
#define NTCR_LISTENERSOCKET_LOG_SHUTDOWN_RECEIVE()                            \
    NTCI_LOG_TRACE("Listener socket "                                         \
...
void ListenerSocket::processSocketReadable(const ntca::ReactorEvent& event)
{
    ...
    if (numIterations > 0) {
        NTCS_METRICS_UPDATE_ACCEPT_ITERATIONS(numIterations);
    }
    ...
    if (error && error != ntsa::Error::e_WOULD_BLOCK) {
        ...
        this->privateRearmAfterAccept(self);
    }
}

void ListenerSocket::processSocketWritable(const ntca::ReactorEvent& event)
{
    NTCCFG_WARNING_UNUSED(event);
}

void ListenerSocket::processSocketError(const ntca::ReactorEvent& event)
{
    NTCCFG_OBJECT_GUARD(&d_object);
...
ntsa::Error ListenerSocket::privateSocketReadableIteration(
    const bsl::shared_ptr<ListenerSocket>& self)
{
    ...
    error = this->privateDequeueBacklog(self, &streamSocket);
    ...
    NTCR_LISTENERSOCKET_LOG_ACCEPT_QUEUE_FILLED(d_acceptQueue.size());
    ...
    while (true) {
        ...
        NTCR_LISTENERSOCKET_LOG_ACCEPT_QUEUE_DRAINED(d_acceptQueue.size());
    ...
    if (d_acceptQueue.authorizeLowWatermarkEvent()) {
        NTCR_LISTENERSOCKET_LOG_ACCEPT_QUEUE_LOW_WATERMARK(
        ...
        if (d_session_sp) {
            ...
            ntcs::Dispatch::announceAcceptQueueLowWatermark(d_session_sp,
                                                            self,
                                                            event,
                                                            d_sessionStrand_sp,
                                                            d_reactorStrand_sp,
    ...
    if (d_acceptQueue.authorizeHighWatermarkEvent()) {
        NTCR_LISTENERSOCKET_LOG_ACCEPT_QUEUE_HIGH_WATERMARK(
    ...
    bool                                   defer)
{
    ...
    if (context.shutdownSend()) {
        ...
        // NTCR_LISTENERSOCKET_LOG_SHUTDOWN_SEND();
    ...
    if (context.shutdownReceive()) {
        ...
        NTCR_LISTENERSOCKET_LOG_SHUTDOWN_RECEIVE();
...
ntsa::Error ListenerSocket::privateCloseFlowControl(
    const bsl::shared_ptr<ListenerSocket>& self,
    bool                                   defer)
{
    ...
    const bsl::shared_ptr<ListenerSocket>& self)
{
    ...
    if (NTCCFG_UNLIKELY(d_acceptRateLimiter_sp)) {
        ...
        if (NTCCFG_UNLIKELY(d_acceptRateLimiter_sp->wouldExceedBandwidth(now)))
        {
            ...
            NTCR_LISTENERSOCKET_LOG_BACKLOG_THROTTLE_APPLIED(timeToSubmit);
...
ntsa::Error ListenerSocket::privateDequeueBacklog(
    const bsl::shared_ptr<ListenerSocket>& self,
    bsl::shared_ptr<ntci::StreamSocket>*   result)
{
    ...
    bsl::shared_ptr<ntsi::StreamSocket> streamSocketBase;
    error = d_socket_sp->accept(&streamSocketBase, d_allocator_p);

    if (NTCCFG_UNLIKELY(error)) {
        if (NTCCFG_LIKELY(error == ntsa::Error::e_WOULD_BLOCK)) {
            NTCR_LISTENERSOCKET_LOG_BACKLOG_UNDERFLOW();
            return error;
        }
        else {
                ...
                error == ntsa::Error::e_CONNECTION_RESET)
            {
            ...
            else if (error == ntsa::Error::e_LIMIT) {
                ...
                NTCR_LISTENERSOCKET_LOG_ACCEPT_LIMIT(error);
                ...
                NTCR_LISTENERSOCKET_LOG_ACCEPT_FAILURE(error);
                return error;
            }
        }
    }
    ...
    if (error) {
        // NTCR_LISTENERSOCKET_LOG_ACCEPTED_SOURCE_ENDPOINT_FAILED(
    ...
    if (error) {
        // NTCR_LISTENERSOCKET_LOG_ACCEPTED_REMOTE_ENDPOINT_FAILED(
        //     streamSocketBase->handle(),
        //     error);
        // NTCS_METRICS_UPDATE_ACCEPT_FAILURE();
        return ntsa::Error(ntsa::Error::e_WOULD_BLOCK);
    ...
    if (error) {
        NTCR_LISTENERSOCKET_LOG_ACCEPTED_SOCKET_OPTIONS_FAILED(
            streamSocketBase->handle(),
            error);
        NTCS_METRICS_UPDATE_ACCEPT_FAILURE();
        return ntsa::Error(ntsa::Error::e_WOULD_BLOCK);
    ...
    NTCR_LISTENERSOCKET_LOG_ACCEPT_RESULT(remoteEndpoint);

    bsl::shared_ptr<ntci::Reactor> reactor =
        reactorPoolRef->acquireReactor(d_options.loadBalancingOptions());
    ...
    streamSocket.createInplace(d_allocator_p,
                               ...
                               resolver,
                               reactor,
                               reactorPoolRef.getShared(),
                               metrics,
                               d_allocator_p);

    error = streamSocket->registerManager(d_manager_sp);
    if (error) {
        NTCR_LISTENERSOCKET_LOG_ACCEPTED_SOCKET_IMPORT_FAILED(
            streamSocketBase->handle(),
            error);
        NTCS_METRICS_UPDATE_ACCEPT_FAILURE();
        streamSocket->close();
...
void ListenerSocket::privateRearmAfterAccept(
    const bsl::shared_ptr<ListenerSocket>& self)
{
    if (d_oneShot) {
        if (!d_acceptQueue.isHighWatermarkViolated()) {
    ...
    const bsl::shared_ptr<ntsi::ListenerSocket>& listenerSocket)
{
    ...
    if (!d_options.sourceEndpoint().isNull()) {
        NTCR_LISTENERSOCKET_LOG_BIND_ATTEMPT(
        ...
        error = listenerSocket->bind(d_options.sourceEndpoint().value(),
                                     d_options.reuseAddress());
        if (error) {
            NTCR_LISTENERSOCKET_LOG_BIND_FAILURE(
                d_options.sourceEndpoint().value(),
                error);
    ...
    const ntci::BindCallback&              bindCallback)
{
    ...
    if (!error) {
        NTCR_LISTENERSOCKET_LOG_BIND_ATTEMPT(endpoint,
                                             d_options.reuseAddress());

        error = d_socket_sp->bind(endpoint, d_options.reuseAddress());
        if (error) {
            NTCR_LISTENERSOCKET_LOG_BIND_FAILURE(endpoint, error);
        }
    }
