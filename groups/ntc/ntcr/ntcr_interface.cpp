...
, d_allocator_p(bslma::Default::allocator(basicAllocator))
{
    ...
    if (d_config.driverMetrics().valueOr(NTCCFG_DEFAULT_DRIVER_METRICS)) {
        ...
        ntcs::MonitorableUtil::registerMonitorable(d_reactorMetrics_sp);
    }
...
Interface::~Interface()
{
    ...
    if (d_reactorMetrics_sp) {
        ntcs::MonitorableUtil::deregisterMonitorable(d_reactorMetrics_sp);
    }
...
bsl::size_t Interface::numSockets() const
{
    ...
    const bsl::size_t numReactors = d_reactorVector.size();

    for (bsl::size_t i = 0; i < numReactors; ++i) {
        const bsl::shared_ptr<ntci::Reactor>& reactor = d_reactorVector[i];
        numSockets += reactor->numSockets();
    }
