...
class EndpointOptions
{
  ...
  public:
    /// Create new endpoint options having the default value. Optionally
    /// specify a 'basicAllocator' used to supply memory. If 'basicAllocator'
    /// is null, the currently installed default allocator is used.
    explicit EndpointOptions(bslma::Allocator* basicAllocator = 0);

    /// Create new endpoint options having the same value as the specified
    ...
    NTSCFG_TYPE_TRAIT_ALLOCATOR_AWARE(EndpointOptions);
};
...
NTSCFG_INLINE
EndpointOptions::EndpointOptions(bslma::Allocator* basicAllocator)
: d_ipAddressFallback()
, d_ipAddressType()
, d_ipAddressSelector()
...
NTSCFG_INLINE
void EndpointOptions::setIpAddressFilter(const ntsa::IpAddressFilter& value)
{
...
NTSCFG_INLINE
void EndpointOptions::setPortFilter(const ntsa::PortFilter& value)
{
    d_portFilter = value;
}
...
NTSCFG_INLINE
const bdlb::NullableValue<ntsa::IpAddressFilter>& EndpointOptions::
...
NTSCFG_INLINE
const bdlb::NullableValue<ntsa::PortFilter>& EndpointOptions::portFilter()
    const
{
    return d_portFilter;
}
