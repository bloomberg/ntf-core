...
BSLS_IDENT_RCSID(ntsa_port_cpp, "$Id$ $CSID$")
...
bsl::size_t PortUtil::format(char*       destination,
                             bsl::size_t capacity,
                             ntsa::Port  port)
{
    bsl::uint16_t value = port;
    bsl::size_t   size  = 0;
...
bool PortUtil::parse(ntsa::Port* result, const char* source, bsl::size_t size)
{
    const char* p = source;
