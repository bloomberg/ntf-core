...
int main()
{
    ...
    // Find the first network interface that supports IPv4 multicasting, but
    // only run this example on machines that support IPv4 multicasting.
    ...
    error = serverOne->bind(ntsa::Endpoint(ntsa::Ipv4Address::any(),
                                           MULTICAST_PORT),
                            true);
    BSLS_ASSERT_OPT(!error);
    ...
    error = serverTwo->bind(ntsa::Endpoint(ntsa::Ipv4Address::any(),
                                           MULTICAST_PORT),
                            true);
    BSLS_ASSERT_OPT(!error);
...

    {
        ...
        BSLS_ASSERT_OPT(context.endpoint().value() ==
